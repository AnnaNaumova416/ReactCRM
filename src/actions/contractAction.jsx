...
export function _generatePDF(data,callback){
	
	let token = localStorage.getItem('accessToken');
	/*let url = "/generatePdf";*/
	let url = "/contracts/generatepdf?access_token=" + token;
	let BASE = "http://172.104.167.150:9082"
	let actualurl = BASE + url;
	
	PDFAPI._callAPI(actualurl, 'POST', data, (type,dt) => {
		if(type == 'success'){
			dispatcher.dispatch({
			    type:'Loader',
			    showLoader: false 
			...
			localStorage.setItem("pdffilename", dt.filename);
...
export function _viewPdf(data){
	let token = localStorage.getItem('accessToken');
	let filename = localStorage.getItem('pdffilename');
	
	let url = "/contracts/downloadpdf?filename=" + filename + "&access_token=" + token;
	let BASE = "http://172.104.167.150:9082"
	let actualurl = BASE + url;
...
export function _getContractList(data){
	...
	let url = "/contracts/list?access_token=" + token;
	let BASE = "http://172.104.167.150:9082"
	let actualurl = BASE + url;
